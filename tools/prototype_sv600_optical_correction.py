...
VERTICAL_ONE = 1 << 12
HORIZONTAL_ONE = 1 << 19
# SV600u-x64.dll reads 0.6 from the model configuration and executes
# horizontal = float(float(horizontal) / (1.0 + 0.6 / 100.0)) before Q19
# conversion.  The live 300 dpi A3L context confirms the resulting 4700 rows
# byte-for-byte.  Vertical chromatic offsets are not magnified by this step.
SV600_X_MAGNIFICATION_PERCENT = 0.6
...
def correct(source: np.ndarray, table: np.ndarray, output_lines: int) -> np.ndarray:
    ...
    magnification = 1.0 + SV600_X_MAGNIFICATION_PERCENT / 100.0
    horizontal_adjusted = np.asarray(
        rows[:, 3:].astype(np.float64) / magnification, dtype=np.float32
    )
    horizontal_step = np.trunc(
        horizontal_adjusted * np.float32(HORIZONTAL_ONE)
    ).astype(np.int64)
...
def emit_c_deltas(dll_path: Path, output_path: Path) -> None:
    ...
    vertical_fixed = np.trunc(
        table[:, :3] * np.float32(VERTICAL_ONE)
    ).astype(np.int32)
    magnification = 1.0 + SV600_X_MAGNIFICATION_PERCENT / 100.0
    # Keep both casts performed by FUN_1c3ab280: double division is stored
    # back to float before the float Q19 multiplication and integer truncation.
    horizontal_adjusted = np.asarray(
        table[:, 3:].astype(np.float64) / magnification, dtype=np.float32
    )
    horizontal_fixed = np.trunc(
        horizontal_adjusted * np.float32(HORIZONTAL_ONE)
    ).astype(np.int32)
    fixed = np.concatenate((vertical_fixed, horizontal_fixed), axis=1)
    ...
    lines = [
        ...
        " * Runtime X magnification: 0.6 percent (FUN_1c3ab280).",
